...
use rusty_falcon::{
    ...
    models::{DeviceapiPeriodDeviceDetailsResponseSwagger, MsaspecPeriodError},
};
...
pub async fn alive_hosts(
    falcon: &FalconHandle,
    hosts: PathBuf,
) -> Result<DeviceapiPeriodDeviceDetailsResponseSwagger, Vec<MsaspecPeriodError>> {
    ...
    let host_ids =
        query_devices_by_filter(&falcon.cfg, None, None, None, Some(filter.as_str())).await.;
